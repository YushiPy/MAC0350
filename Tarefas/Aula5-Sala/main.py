...
def get_active_user(session_user: Annotated[str | None, Cookie()] = None) -> User:
    ...
    if session_user is None:
        raise HTTPException(
            status_code=http.client.UNAUTHORIZED, detail="Not logged in."
        )
    ...
    if user is None:
        raise HTTPException(
            status_code=http.client.NOT_FOUND, detail="Invalid session user."
        )
...
def update_bio(
    data: BioUpdateData, user: User = Depends(get_active_user)
) -> dict[str, str]:
